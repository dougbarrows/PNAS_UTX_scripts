...
samplenames <- list.files("../salmon_analysis_1.1.0_NOTtrimmed/quant_../fastq_files/quant_files/") #this is the directory with my Salmon outputs and I wirte their names into a vector 
...
files_quant <- file.path("../salmon_analysis_1.1.0_NOTtrimmed//quant_../fastq_files/quant_files/",colData$samplenames)
...
##########
# look at correlation of genes that change in WTFGFR3 + UTX vs those that change in S249 vs WTGFR3
corr_table <- merge(S249C_EV_vs_WTFGFR3_EV_rnk, WTFGFR3_UTX_vs_WTFGFR3_EV_rnk, by = 1)
colnames(corr_table) = c("gene","S249vWTFGFR3", "UTXvsEV_WTFGFR3")
corr_table[corr_table$gene %in% c("UPK1B", "UPK2", "UPK3B"),]

plot(corr_table$S249vWTFGFR3, corr_table$UTXvsEV_WTFGFR3)

S249alldown_vWT_AllEnh_UTXpeaks_Overlap <- read.table("S249C_dependentGenes_analysis/UTX_enh_overlap_analysis/S249vWTFGFR3/genes_down_with_S249C/down_all_P05/S249alldown_vWT_AllEnh_UTXpeaks_Overlap.txt", stringsAsFactors = F) %>%
  .[,1]

corr_table$UTXenhPeak_S249down_ov <- ifelse(corr_table$gene %in% S249alldown_vWT_AllEnh_UTXpeaks_Overlap, "UTX_enh", "no_ov")

ggplot(corr_table, aes(x = S249vWTFGFR3, y = UTXvsEV_WTFGFR3, color = UTXenhPeak_S249down_ov)) + geom_point()
...
# filter to those genes that go down in EV vs S249
res_S249C_UTX_vs_S249C_EV_sort_df_downgenesEV <- dplyr::filter(res_S249C_UTX_vs_S249C_EV_sort_df, GeneSymbol %in% res_S249C_EV_vs_EV_EV_sort_genes_down)

S249C_UTX_vs_S249C_EV_rnk <- res_S249C_UTX_vs_S249C_EV_sort[, c(7,4)] %>%
  as.data.frame()

##########
# look at correlation of genes that change in S249 + UTX vs those that change in S249 vs WTGFR3
corr_table <- merge(S249C_EV_vs_WTFGFR3_EV_rnk, S249C_UTX_vs_S249C_EV_rnk, by = 1)
colnames(corr_table) = c("gene","S249vWTFGFR3", "UTXvsEV_S249FGFR3")
corr_table[corr_table$gene %in% c("UPK1B", "UPK2", "UPK3B"),]

plot(corr_table$S249vWTFGFR3, corr_table$UTXvsEV_S249FGFR3)
cor(corr_table$S249vWTFGFR3, corr_table$UTXvsEV_S249FGFR3)
...
#   if ((corr_table[i,1] > 5 | corr_table[i,1] < -5) & (corr_table[i,2] > 5 | corr_table[i,2] < -5)){
#     corr_table$big_change <- "stat_high"
#   } else {
#     corr_table$big_change <- "stat_low"
#   }
# }
library(ggpmisc)
ggplot(corr_table, aes(x = S249vWTFGFR3, 
                       y = UTXvsEV_S249FGFR3
                       )) + 
...
ggsave("UTXvEV_S249vWTFGFR3_correlate.pdf", useDingbats=FALSE)
